 ...
 * `dev.pschmitt.jellyfin.models.PendingDownloadRequestDto`) is fulfilled by [PendingDownloadWorker]
 * (via `PendingDownloadFulfiller`) - the season/episode the user queued while it only existed in
 * Sonarr has now appeared in the Jellyfin library and started downloading. Same pattern as
 * [PvrDownloadFinishedNotifier] (lazy channel creation, tap opens the app), kept on a separate
 * channel so users can mute one without the other.
 */
@Singleton
    ...
    fun notifyFulfilled(title: String) {
        ...
        val notification =
            NotificationCompat.Builder(context, CHANNEL_ID)
                ...
                .setContentText(
                    context.getString(CoreR.string.pending_download_fulfilled_notification)
                )
                .setAutoCancel(true)
