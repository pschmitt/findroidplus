 ...
 * User-initiated data transfer jobs are explicitly exempted from the "no starting a new foreground
 * service from a background process" restriction that causes the bug this whole subsystem exists to
 * route around (see the "Real fix for stuck background downloads" plan) - the OS treats "the user
 * asked for this transfer" as sufficient justification on its own. On API < 34 there is no
 * equivalent; [schedule]/[cancel] are no-ops there and the app relies on
    ...
    override fun onStartJob(params: JobParameters?): Boolean {
        try {
            ContextCompat.startForegroundService(
                this,
                Intent(this, VideoDownloadService::class.java),
            )
        } catch (e: Exception) {
    ...
    companion object {
        ...
        fun schedule(context: Context) {
            ...
            val jobInfo =
                JobInfo.Builder(
                        JOB_ID,
                        ComponentName(context, ResumeDownloadsJobService::class.java),
                    )
