 ...
 * meaningfully tied to "Termux" specifically), so a single shared secret the user explicitly copies
 * out is the simpler, equally honest security model.
 */
@Singleton
class LocalControlAuth
@Inject
constructor(private val secureCredentialStore: SecureCredentialStore) {
    private val random = SecureRandom()

    fun getOrCreateToken(): String {
        secureCredentialStore.getString(TOKEN_KEY)?.let {
            return it
        }
