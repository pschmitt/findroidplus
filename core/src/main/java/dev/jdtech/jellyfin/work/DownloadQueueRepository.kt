...
/**
 * In-memory coordination point between [dev.jdtech.jellyfin.utils.DownloaderImpl] (UI-facing
 ...
 * [dev.jdtech.jellyfin.utils.Downloader.reconcilePendingDownloads] re-derive pending requests from
 * that table and re-[enqueue] them here, only from moments guaranteed to be foreground-eligible.
 */
@Singleton
class DownloadQueueRepository @Inject constructor() {
    ...
    /** Requests that don't yet have a job tracked for them - what a (re)starting service should pick up. */
    ...
    fun markSucceeded(sourceId: String) {
        jobs.remove(sourceId)
        requests.remove(sourceId)
        setProgress(sourceId, DownloadProgress(status = DownloadManager.STATUS_SUCCESSFUL, percent = 100))
    }
     ...
     * request pending so it resumes automatically the next time the app is foregrounded or the
     * API 34+ user-initiated job wakes us, but tells the UI to stop implying it's about to start.
     */
    fun markAwaitingForeground(sourceIds: Collection<String>) {
        sourceIds.forEach { setProgress(it, DownloadProgress(status = DownloadProgress.STATUS_AWAITING_FOREGROUND)) }
    }

    fun updateProgress(sourceId: String, value: DownloadProgress) = setProgress(sourceId, value)

    fun progressFlow(sourceId: String): Flow<DownloadProgress> =
        progress.map { it[sourceId] ?: DownloadProgress(status = DownloadManager.STATUS_PAUSED) }
