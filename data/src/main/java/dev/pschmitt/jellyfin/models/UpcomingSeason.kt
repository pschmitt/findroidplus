 ...
 * screen. Sonarr's v3 API has no "season" resource of its own - this is derived by grouping `GET
 * /api/v3/episode` entries by `seasonNumber` (see
 * [dev.pschmitt.jellyfin.repository.matchMissingSeasons]), so [episodeCount] counts every episode
 ...
 * [dev.pschmitt.jellyfin.repository.SeerrRepository.getSeasonPosterUrls]) once the season numbers
