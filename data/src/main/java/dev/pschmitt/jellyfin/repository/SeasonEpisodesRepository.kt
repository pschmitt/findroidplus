 ...
 * [CalendarRepository] (a global date-range view across every show), this is scoped to one series -
 * matching it to a Sonarr series happens internally via tvdbId, the same join
 * [dev.pschmitt.jellyfin.repository.matchSonarrCalendar] uses.
 */
interface SeasonEpisodesRepository {
     ...
     * [knownSeasonNumbers] the season numbers already present in the Jellyfin library for it - any
     * season Sonarr knows about beyond that set is returned. Empty (not an error) when Sonarr isn't
     * configured, the show isn't tracked by Sonarr, or nothing is missing.
     */
