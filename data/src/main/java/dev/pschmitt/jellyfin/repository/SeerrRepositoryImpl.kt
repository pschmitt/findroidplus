...
import dev.pschmitt.jellyfin.api.pvr.SeerrSearchResult
import dev.pschmitt.jellyfin.models.SeerrEpisodeDetail
...
import dev.pschmitt.jellyfin.models.SeerrSeasonDetail
import dev.pschmitt.jellyfin.models.SeerrSeasonInfo
...
class SeerrRepositoryImpl(
    ...
    override suspend fun search(query: String): Result<List<SeerrSearchItem>> = runAction { api ->
        api.search(query).results.mapNotNull { it.toSearchItem() }
    }
    ...
    override suspend fun getDetails(
        ...
        when (mediaType) {
            ...
            SeerrMediaType.TV -> {
                ...
                val season = seasonNumber?.let { requestedSeasonNumber ->
                                ...
                                when (request.media.mediaType) {
                                    SeerrApi.MEDIA_TYPE_MOVIE -> {
                                        ...
                                        SeerrRequestItem(
                                            ...
                                            title =
                                                details.title.ifBlank {
                                                    "TMDB ${request.media.tmdbId}"
                                                },
                                            posterUrl = details.posterPath?.toPosterUrl(),
                                            mediaStatus =
                                                SeerrMediaStatus.fromCode(request.media.status),
                                        )
                                    }
     ...
     * that season is its free-text `name` (e.g. "Season 3 Official Trailer") - [seasonNameMatch] is
     * a best-effort match on that, tried before falling back to the show-wide pick. TMDB also lists
     * every kind of video (teasers, clips, featurettes, ...) with no reliable ordering, so within
     * each candidate pool an explicit "Trailer" is preferred, falling back to the first YouTube
     * video at all so there's still something to play if none is tagged "Trailer".
     */
    private fun List<SeerrRelatedVideo>.firstTrailerUrl(seasonNumber: Int?): String? {
        val youtubeVideos = filter {
            it.site.equals("YouTube", ignoreCase = true) && it.url != null
        }
        if (seasonNumber != null) {
            val seasonNameMatch = Regex("""(?i)\bseason\s*0*$seasonNumber\b|\bs0*$seasonNumber\b""")
            val seasonScoped = youtubeVideos.filter {
                it.name?.let(seasonNameMatch::containsMatchIn) == true
            }
            seasonScoped.bestMatch()?.let {
                return it
            }
        }
    ...
    private suspend fun <T> runAction(block: suspend (SeerrApi) -> T): Result<T> {
        val api = api() ?: return Result.failure(IllegalStateException("Seerr is not configured"))
        return try {
