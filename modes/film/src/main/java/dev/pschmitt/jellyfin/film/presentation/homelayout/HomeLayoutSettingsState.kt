 ...
 * depends on - e.g. Sonarr/Radarr for "Pending downloads", Seerr for the Discover rows - shown next
 * to the label so it's clear at a glance which integration a section needs.
 */
data class HomeLayoutRow(
    val key: String,
    val label: UiText,
    val serviceIcons: List<Int> = emptyList(),
)
