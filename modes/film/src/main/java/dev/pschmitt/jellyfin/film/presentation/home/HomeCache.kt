 ...
 * `init` so a cold app start paints the last-known Home content on the very first frame instead of
 * a blank screen while the real network round trip is still in flight. [HomeViewModel.loadData]
 ...
 * (continue watching, next up) changes far more often than the calendar does, so there's no "still
 * fresh enough to skip the refresh" window here, only "instant vs. blank while it loads".
 */
@Singleton
